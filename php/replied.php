...
header("Access-Control-Allow-Headers: Content-Type, Access-Control-Allow-Headers, Authorization, X-Requested-With");
  ...
  $con = mysqli_connect('localhost','root','','id10505005_geopusara');
    ...
    $namecheckquerry = "SELECT * FROM enquiry WHERE name ='".$insertnama."' AND email = '".$insertemail."'";
    $namecheck = mysqli_query($con,$namecheckquerry) or die ("2: name check querry failed");

    $deleteuserquerry = "DELETE FROM enquiry WHERE name='".$insertnama."' AND email = '".$insertemail."' ";
